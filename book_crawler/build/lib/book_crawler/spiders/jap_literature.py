...
class Bookinfo_spider(scrapy.Spider):
    ...
    def parse_bookinfo(self, response):
        ...
        author_intro = sel.xpath(
            "//div[@class='mod_b type02_m057 clearfix'][2]/div/div/strong[2]/preceding-sibling::text()").re(r'\w+\S+\n*')
        # 判斷如果作者介紹結果為空則換從第一個strong定位抓取下面資料
        if len(author_intro) == 0:
            author_intro = sel.xpath(
                "//div[@class='mod_b type02_m057 clearfix'][2]/div/div/strong[1]/following-sibling::text()").re(r'\w+\S+\n*')
        ...
        if len(translator_name) == 0:
            ...
            bookinfo['translator_name'] = translator_name
        bookinfo['translator_intro'] = sel.xpath(
            "//div[@class='mod_b type02_m057 clearfix'][2]/div/div/strong[2]/following-sibling::text()").re(r'\w+\S+\n*')
